...
// .wrangler/tmp/bundle-Ig2giw/checked-fetch.js
...
// .wrangler/tmp/bundle-Ig2giw/strip-cf-connecting-ip-header.js
...
// src/repositories/D1SplitRepository.js
var D1SplitRepository = class {
  constructor(db, config = {}) {
    this.db = db;
    this.config = {
      maxParticipants: parseInt(config.maxParticipants) || 50,
      maxExpenses: parseInt(config.maxExpenses) || 500,
      maxTotalSplits: parseInt(config.maxTotalSplits) || 1e4,
  ...
  async create() {
    ...
    await this.db.prepare("INSERT INTO splits (id, created_at) VALUES (?, ?)").bind(id, createdAt).run();
   ...
   */
  async findById(id) {
    const split = await this.db.prepare("SELECT id, created_at as createdAt FROM splits WHERE id = ?").bind(id).first();
    if (!split) {
      return null;
    }
    ...
    const expenses = await this.db.prepare(
  ...
  async addParticipant(splitId, name) {
    const split = await this.db.prepare("SELECT id FROM splits WHERE id = ?").bind(splitId).first();
    if (!split) {
      throw new Error("Split not found");
    }
    const count = await this.db.prepare("SELECT COUNT(*) as count FROM participants WHERE split_id = ?").bind(splitId).first();
  ...
  async addExpense(splitId, participantId, description, amount) {
    ...
    const participant = await this.db.prepare("SELECT id FROM participants WHERE id = ? AND split_id = ?").bind(participantId, splitId).first();
    if (!participant) {
      throw new Error("Participant not found");
    }
    const count = await this.db.prepare("SELECT COUNT(*) as count FROM expenses WHERE split_id = ?").bind(splitId).first();
    ...
    ).bind(id, splitId, participantId, description, amount).run();
    return expense;
  }
  /**
  ...
  async deleteExpense(splitId, expenseId) {
    ...
    const expense = await this.db.prepare("SELECT id FROM expenses WHERE id = ? AND split_id = ?").bind(expenseId, splitId).first();
    if (!expense) {
      throw new Error("Expense not found");
    }
    await this.db.prepare("DELETE FROM expenses WHERE id = ?").bind(expenseId).run();
    return true;
  }
  /**
  ...
  async updateParticipantStatus(splitId, participantId, isDone) {
    ...
    ).bind(participantId, splitId).first();
    if (!participant) {
      throw new Error("Participant not found");
    }
    await this.db.prepare("UPDATE participants SET is_done = ? WHERE id = ?").bind(isDone ? 1 : 0, participantId).run();
    return {
      id: participant.id,
  ...
  async getActiveSplitsCount() {
    const result = await this.db.prepare("SELECT COUNT(*) as count FROM splits").first();
    return result.count;
  }
  /**
   * Delete split
  ...
  async cleanupExpiredSplits() {
    ...
    const result = await this.db.prepare("DELETE FROM splits WHERE created_at < ?").bind(expiryDate.toISOString()).run();
    return result.changes || 0;
  }
};
__name(D1SplitRepository, "D1SplitRepository");

// src/middleware/validation.js
var MAX_NAME_LENGTH = 100;
var MAX_DESCRIPTION_LENGTH = 200;
var MAX_AMOUNT = 1e6;
async function validateSplitId(c, next) {
...
__name(validateSplitId, "validateSplitId");
async function validateParticipant(c, next) {
  let body;
  try {
...
__name(validateParticipant, "validateParticipant");
async function validateExpense(c, next) {
  let body;
  try {
    body = await c.req.json();
  } catch {
...
function createSplitRoutes() {
  const router = new Hono2();
  const getRepository = /* @__PURE__ */ __name((c) => {
  ...
  router.post("/", async (c) => {
    ...
    const maxSplits = parseInt(c.env.MAX_TOTAL_SPLITS) || 1e4;
  ...
  router.get("/:id/settlement", validateSplitId, async (c) => {
    ...
    const allDone = split.participants.length > 0 && split.participants.every((p) => p.isDone);
    if (!allDone) {
      return c.json({
        ready: false,
...
__name(createSplitRoutes, "createSplitRoutes");
...
// .wrangler/tmp/bundle-Ig2giw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Ig2giw/middleware-loader.entry.ts
