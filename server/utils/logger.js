...
if (appInsightsClient) {
  try {
    const { AzureApplicationInsightsLogger } = await import('winston-azure-application-insights');
    logger.add(new AzureApplicationInsightsLogger({
      client: appInsightsClient,
      level: 'info'
    }));
  } catch (error) {
    console.warn('Failed to initialize Application Insights transport:', error.message);
  }
}
